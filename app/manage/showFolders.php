...
<?include("../_jqxGridInclude.php");?>
 <script language="javascript">
        ...
        function loadGrid(locationSeq){
            ...
            var source =
            {
                ...
                datafields: [
                    ...
                    { name: 'spcbSynchedDate', type: 'date' },
            ...
            var actions = function (row, columnfield, value, defaulthtml, columnproperties) {
                    ...
                    if(!hiddenFlag){
                    	    html = "<div style='text-align: center; margin-top: 5px;'><a href='javascript:Edit("+ data['seq'] + ")' ><i class='fa fa-pencil-square-o' title='Edit'></i></a>";
                        	html += "<span style='margin-left:5px;'><a href='javascript:goToCalibration("+ data['seq'] + ', \"' +  data["m2msitecode"] + "\")' ><i class='fa fa-building-o' title='Calibration'></i></a></span></div>";
                    }else{
                    	    html = "<div style='text-align: center; margin-top: 5px;'><a href='javascript:goToCalibration("+ data['seq'] + ', \"' +  data["m2msitecode"] + "\")' ><i class='fa fa-building-o' title='Calibration'></i></a></div>";
                    }
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
                  ...
                  { text: 'Vendor',  datafield: 'vendor',width: "9%"},
                  { text: 'Status',  datafield: 'isonline', width: "6%" },
                  { text: 'm2mCode', hidden:true,  datafield: 'm2msitecode'},
                  ...
                  { text: 'SPCB Synced Till', sortable:false, datafield: 'spcbSynchedDate',width: "16%",cellsformat: 'dd-MM-yyyy hh:mm:ss tt' },				  				  
                  { text: 'Actions',   datafield: 'action', width: "9%", renderer:columnrenderer,cellsrenderer:actions},
                ]
            });
        }

        function goToCalibration(seq,siteCode){
           	location.href = "calibrationStation.php?folderSeq="+seq+"&siteCode="+siteCode;
        }
          
        function changeStatus(seq,isEnabled){
            $("#enableDiv"+seq).html("<i class='fa fa-spinner' aria-hidden='true' title='wait'>")
            url = "showFolders.php?call=changeStatus&editSeq="+seq+"&isEnabled="+isEnabled;
        ...
        function changeVisible(seq,isVisible){
            ...
            url = "showFolders.php?call=changeVisible&editSeq="+seq+"&isEnabled="+isVisible;
        	$.getJSON(url, function( data ) {
            	if(data.result == "success"){
                	if(isVisible == "0"){
        ...
        function Delete(seq,folderName){
             deleteObj(seq,folderName,"Folder","showFolders.php");
        }
