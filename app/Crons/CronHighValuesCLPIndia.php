<?php 
  ...
  $CLP_LOCATION_SEQ = 18;
  $highValueRules = $HVRDS->FindAllForHighValueCron(true,$CLP_LOCATION_SEQ);
  ...
  foreach($highValueRules as $highValueRule){
     ...
     echo("\n<br> Rule for CLP - station ". $highValueRule->getIndustryName(). " : " . $highValueRule->getStationName() ." (Type : - " . $highValueRule->getStationType() . ") for channel ". $highValueRule->getChannelName());
	 ...
	 echo ("<br> - Average value found :". $avgValue . " from seq ". $lastWQDSeq ." to Seq :- " . $maxWQD);
	 if($maxWQD != 0){
