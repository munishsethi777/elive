<?php
  ...
  class WQDDataDataStore  {
    ...
    private static $FIND_FOLDER = "SELECT * FROM wqdfiledata inner join folder on wqdfiledata.wqdfolderseq =  folder.seq where wqdfolderseq = :folderseq and wqdfiledata.wqdfiledataseq > :lastsyncedseq order by wqdfiledataSeq LIMIT ";
    private static $FIND_FOLDER_LATEST = "SELECT * FROM wqdstackfiledata where wqdfolderseq = :folderseq order by wqdfiledataSeq DESC LIMIT 1";
   ...
   public function getChannelsLatestInfo($folderSeq){
    ...
    public function getChannelInfoData($fromDate, $folderSeq, $channelNos,$rows){
   ...
   public function getTotalCount($fromDate, $toDate, $folderSeq, $channelNos,$interval){
   	$sql = "Select count(*) from wqdfiledata where wqdfolderseq = :folderseq";
   	$sql .= " and wqdfiledatadated >= '" . $fromDate . "'";
...
wqdfiledataseq > :lastSeq and wqdfolderseq = :folSeq order by wqdfiledataseq DESC";
      ...
      public static function getAverageDataByDataArrayDataSlices($timeBase,$dataArray,$dateSlices,$channelsNamesArray,$valueType,$dataFinal,$folSeq=0){
      	...
      	if(!empty($dataArray)){
      		foreach($dataArray as $dateDataLong => $data){
      			...
      			$dslDated = date("d-m-Y H",$dateSliceLong);
      			$sliceHours = date("H",$dateSliceLong);
      			$dataDated1 = date("d-m-Y H",$dateDataLong);
      			$pageNum = $_GET["pagenum"];
      			if($pageNum >1 && $flag){
      				$pageNum = $_GET["pagenum"];
      				$dataDated = new DateTime();
      				$dataDated->setTimestamp($dateDataLong);
      				$dateSlice1 = new DateTime($dateSlices[$dateSliceIndex]);
      				while ($dateSlice1 < $dataDated && ($sliceHours != 23)){
      					$dateSliceIndex++;
      					$dateSlice1 = new DateTime($dateSlices[$dateSliceIndex]);
      				}
      				$flag = false;
      			...
      			if($dateSliceLong != false){
      				$datesDiffMinutes = round(abs($dateDataLong - $dateSliceLong) / 60,2);
      				if($datesDiffMinutes >= $minsDiff){
      					if(count($totDataForSlice) > 0){
      						$avgData = self::getChannelAverageValues($totDataForSlice,$channelsNamesArray,$valueType,$folSeq);
      						if($avgData != NULL){
      				...
      				if($dateDataDate >= $dateSliceDate){
      					...
      					if($avgData != NULL){
      						$dat = date("d-m-Y H:i",$dateSliceLong);
      						if(!in_array($dat, $dateArr)){
      						    $arr = self::appendArray($dataFinal[$dat],$avgData);
      						    $dataFinal[$dat] = $arr;
      						    array_push($dateArr,$dat);
      						}
      					}
      					$totDataForSlice = array();
      					$dateSliceIndex++;
...
//            	if(!empty($dataArray)){
// 	           foreach($dataArray as $dateDataLong => $data){
// 	       	    	$dataDated = date("d-m-Y H:i:s",$dateDataLong);
// 	            	$dateSlice1 = $dateSlices[$dateSliceIndex];
// 	                $dateSliceLong = strtotime($dateSlices[$dateSliceIndex]);
// 	                  if($dateSliceLong != false){
// 	                    $datesDiffMinutes = round(abs($dateDataLong - $dateSliceLong) / 60,2);
// 	                    if($datesDiffMinutes >= $minsDiff){
...
//         }
        ...
        private static function getChannelAverageValues($dateSliceDataArray,$channelNamesArr,$valueType,$folSeq = 0){
...
//                     $eachDataPU = 0;
//                 }      
                    if($chStatus == 0 || $chStatus == 128 || $chStatus == 129){
                        $eachDataPU = ConvertorUtils::getPrescribedValue($chName,$data);
                    }else if($data == 0){
                        $eachDataPU = 0;
                    }  
        ...
        public function getChannelData($fromDate, $folderSeq, $channelNos,$rows){
		  	$sql = "Select wqdfiledataseq,wqdfiledatadated,";
		  	for ($i = 0, $l = count($channelNos); $i < $l; ++$i) {
		  		$sql .= "ch". $channelNos[$i] ."value";
		  		$sql .= ",ch". $channelNos[$i] ."status";
