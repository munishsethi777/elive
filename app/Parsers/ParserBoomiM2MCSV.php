...
class ParserBoomiM2MCSV{
	public static function parse($filePath,$folderSeq){
		...
		$WQDArray = array();
        $valueIndex = 6;
        if(($folderSeq == 307) || ($folderSeq == 241) || ($folderSeq == 251) || 
            ($folderSeq == 252) || ($folderSeq == 253) || ($folderSeq == 254)){ //vayunanda/indosponge processed value is higher than scaled value, so pick scaled value only
            $valueIndex = 7;
        }
        while(!feof($file))
        {
