<?php
class PPCBService {
    private static $ppcbService;
    private static $logger;
    private $chData;
    public static function getInstance() {
        if(empty(self::$ppcbService)){
            self::$ppcbService = new PPCBService();
            self::$logger = Logger::getLogger("eliveLogger");
        }
        return self::$ppcbService;
    }
    ...
    public function excuteApiCall($dataArr,$station,$parentStationSeq){
        ...
        //key for Medicare ludhiana
        $apiKey = "MTkxMDIwMjBfYmhvb21pX2FuYWx5emVyXzEyNTYzNQ==";
        $auth_header = 'Authorization: Basic ' . $apiKey;
        //	echo "<br>". $auth_header . '<br>';
        $ch = curl_init();
        
        $url = "http://ppcbcems.nic.in/ppcpcb-api/api/industry/$locationSeq/station/$stationSeq/data";
        ...
        if($status == 1 && $msg == "Success"){
            echo ("<br>Data Synced Successfully for Medicare,LDH Station");
            ...
            if(!empty($lastSeq)){
                ...
                self::$logger->error("Failed to push data for punjab control board for station seq - " . $stationSeq .  ". Error - " . $msg);
            }
        }
